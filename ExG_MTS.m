% Algorithm ExG_MTS:  ExG(Excess Green Vegetation Index)
% With Manual Threshold Selection (MTS) Value 

clear;

% System variables
NumberOccurrences = 90.0; % Manual Threshold Selection (MTS)
...
%4) Results ----------------------------------------------------------------
subplot(2,2,1); imhist(ExG); title(['Threshold value:P=(',num2str(NumberOccurrences), '/',num2str(TotalNumberPixels), ')=',num2str(ValueThreshold)]);
subplot(2,2,2); imshow(imBinarized); title('binary Image (ExG MTS)');
