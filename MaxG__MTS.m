% Algorithm MaxG_MTS:  MaxG(Maximum Green Index)
% With Manual Threshold Selection (MTS) Value 

clear;

% System variables
NumberOccurrences = 14.0; % Manual Threshold Selection (MTS)
...
imBinarized = imbinarize(MaxG,ValueThreshold); 
...
%4) Results ----------------------------------------------------------------
subplot(2,2,1); imhist(MaxG); title(['Threshold value:P=(',num2str(NumberOccurrences), '/',num2str(TotalNumberPixels), ')=',num2str(ValueThreshold)]);
subplot(2,2,2); imshow(imBinarized); title('binary Image (MaxG MTS)');
